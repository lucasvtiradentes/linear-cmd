...
export function readJson<T = Record<string, unknown>>(filePath: string): T {
  ...
  try {
    ...
    throw new Error(`Failed to parse JSON file: ${filePath}. Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export function writeJson<T>(filePath: string, data: T, pretty = true): void {
  try {
    ...
    throw new Error(`Failed to write JSON file: ${filePath}. Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}